...
export default function Recipes() {
    ...
    const [likes, setLikes] = useState(recipes.map(recipes => recipes.likes));
    const [isSaved, setIsSaved] = useState(recipes.map(() => false));
        ...
        <section className="mb-10 flex flex-col justify-center">
            <div className="flex flex-wrap gap-16 tlg:gap-7 tsm:gap-9 txl:gap-10 mt-5 tsm:mt-0 tsm:flex-col">
                {recipes.map((recipe, index) => (
                    <div className='w-[43%] tsm:w-full lg:w-[45%] txl:w-[44%] tlg:w-[46%] tsm:px-2 mb-2' key={recipe.id}>
                        <div className='flex justify-center'>
                            <div className='flex max-w-[24rem] h-11 items-center'>
                                <p className='text-[1.65rem] tsm:text-2xl tlg:text-2xl lg:text-2xl receita text-center cursor-pointer'>{recipe.title}</p>
                            </div>
                        </div>
                        ...
                        <div className='items-center flex justify-between mt-6 px-2'>
                            <div className='flex gap-1 tsm:items-center'>
                                ...
                                <p className='inter text-customGray text-[0.95rem] tsm:text-xs'>{recipe.rating.toFixed(1)}</p>
                            </div>
                            <p className=' inter text-customGray text-[0.95rem] leading-tight tsm:text-xs'>{recipe.date}</p>
                        </div>
