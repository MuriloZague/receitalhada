...
class UserController extends Controller<Users> {
  ...
  public async createUser(req: Request, res: Response): Promise<Response> {
    ...
    const result = await this.handler(async () => {
      ...
      await sendMail(
        data.email,
        'Confirme seu email',
        'Código de autentificação: ' + authenticate_code,
      );
  ...
  public async listAllUsers(req: Request, res: Response): Promise<Response> {
    ...
    const result = await this.handler(async () => {
      // Se informou ordem
      if (order) {
        let [orderField, orderDirection]: string[] = (order as string)
          .toLowerCase()
          .split(';');

        // O campo precisa fazer parte dos campos da model
        if (fields.includes(orderField)) {
          if (!['asc', 'desc'].includes(orderDirection)) orderDirection = 'asc';

          // Monta string com o campo e a direção (padrão: asc)
          const orderString = JSON.parse(
            `{"${orderField}": "${orderDirection}"}`,
          );
          orderBy.push(orderString);
        } else {
          throw new AppError(
            `The property "${orderField}" does not exist in the model`,
  ...
  public async authenticateUser(
    ...
    const result = await this.handler(async () => {
      ...
      if (data.email)
        ...
        throw new AppError(
          'The authentication credentials have not been entered',
          ErrorCode.VALIDATION_ERROR,
        );
      ...
      if (!passwordsIsEquals)
        throw new AppError(
          "The entered password is not equal to the user's password",
          ErrorCode.VALIDATION_ERROR,
        );
  ...
  public async authenticateEmailCode(
    req: Request,
    res: Response,
  ): Promise<Response> {
    const result = await this.handler(async () => {
      ...
      if (!authenticate_code)
        throw new AppError(
          'Authenticate code is not found',
          ErrorCode.VALIDATION_ERROR,
        );

      const user = await prisma.users.findUniqueOrThrow({
        where: { authenticate_code },
      });

      if (user.email != email)
        throw new AppError(
          'Authenticate code is invalid',
          ErrorCode.VALIDATION_ERROR,
        );
  ...
  public async sendForgotPasswordCode(
    req: Request,
    res: Response,
  ): Promise<Response> {
    ...
    const result = await this.handler(async () => {
      ...
      await sendMail(
        ...
        'Código de restauração de senha: ' + forgotPasswordCode,
      );
      ...
      await prisma.$transaction(async (prisma) => {
        ...
        if (
          user.forgot_password_attempted_at &&
          ...
          if (user.forgot_password_attempts >= MAX_ATTEMPTS) {
            throw new AppError(
              'You have exceeded password recovery attempts',
              ErrorCode.VALIDATION_ERROR,
            );
          }
        }
      ...
      return {
        message:
          'An authentication code was sent to your email, please check your inbox',
      };
    });
  ...
  public async authenticateForgotPasswordCode(
    req: Request,
    res: Response,
  ): Promise<Response> {
    const NOW = new Date();
    ...
    const result = await this.handler(async () => {
      ...
      return {
        message:
          "User's forgot password request was accepted! Password was edited",
      };
    });
  ...
  public async editPassword(req: Request, res: Response): Promise<Response> {
    ...
    const result = await this.handler(async () => {
      ...
      await sendMail(
        data.email,
        'Restauração de senha',
        'Código de resturação de senha: ' + forgotPasswordCode,
      );

      return {
        message:
          'A authentication code was send in your email, please check your inbox',
      };
    });
  ...
  public validate(data: any) {
    ...
    if (data.email) {
      ...
      if (emailSplited.length < 2 || emailSplited[1].trim() == '')
        throw new AppError(
          'Invalid email format. Please ensure you enter a valid email address, such as example@domain.com',
          ErrorCode.VALIDATION_ERROR,
        );
    }
    ...
    if (data.phone) {
      ...
      if (data.phone.length !== 11)
        throw new AppError(
          'Invalid phone format. Please ensure your enter a valid phone number, such as (11)99988776655',
          ErrorCode.VALIDATION_ERROR,
        );
    }
    ...
    if (data.password) {
      ...
      if (!regexPattern.test(data.password))
        throw new AppError(
          'Invalid password, must contain at least 8 characters, 1 uppercase and lowercase letter, 1 number and 1 special letter',
          ErrorCode.VALIDATION_ERROR,
        );
    }
  }
